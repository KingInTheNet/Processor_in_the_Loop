function res = model_sim(args)
    ...
    arguments
        ...
        si = Simulink.SimulationInput('SILS_20241709');
        ...
        function locPostStepFcn(simTime)
            so = simulink.compiler.getSimulationOutput('SILS_20241709');
            res = extractResults(so, prevSimTime);
            stopRequested = feval(args.OutputFcn, simTime, res);
            if stopRequested
                simulink.compiler.stopSimulation('SILS_20241709');
            end
            prevSimTime = simTime;
        end
